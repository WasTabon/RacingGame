...
public class Iteration9Setup
{
    ...
    [MenuItem("RacingGame/Iteration 9 - Standings")]
    public static void Build()
    {
        EnsureFolder("Assets/RacingGame/Scenes");

        Scene st = EditorSceneManager.NewScene(NewSceneSetup.EmptyScene, NewSceneMode.Single);
        BuildStandingsScene();
        string stPath = "Assets/RacingGame/Scenes/Standings.unity";
        EditorSceneManager.SaveScene(st, stPath);

        List<string> scenes = new List<string>();
        string[] prior = { "MainMenu", "Hub", "Staff", "RnD", "Car", "Base", "Weekend", "Finance" };
        ...
        EditorSceneManager.OpenScene(stPath);
        AssetDatabase.SaveAssets();
        Debug.Log("RacingGame Iteration 9 (Standings) setup complete. Enter via Hub reputation chip, or open Standings scene and Play.");
    }

    private static void BuildStandingsScene()
    {
        CreateCamera();
        CreateEventSystem();
        CreateManagers();
        ...
        GameObject ctrlGo = new GameObject("StandingsController");
        ctrlGo.transform.SetParent(canvas.transform, false);
        ctrlGo.AddComponent<RectTransform>();
        StandingsController ctrl = ctrlGo.AddComponent<StandingsController>();
    ...
    private static void BuildTopBar(Transform safe, StandingsController ctrl)
    {
        ...
        TMP_Text title = CreateText(hud.transform, "Title", "STANDINGS", 42, Color.white, TextAlignmentOptions.Left);
        SetRect(title.rectTransform, new Vector2(0, 1), new Vector2(0, 1), new Vector2(0, 1), new Vector2(420, 56), new Vector2(124, -22));
        title.fontStyle = FontStyles.Bold;

        TMP_Text season = CreateText(hud.transform, "Season", "", 24, ColorMuted, TextAlignmentOptions.Right);
        SetRect(season.rectTransform, new Vector2(1, 1), new Vector2(1, 1), new Vector2(1, 1), new Vector2(420, 40), new Vector2(-24, -56));

        ctrl.backButton = back;
        ctrl.seasonText = season;
    }

    private static void BuildSubTabs(Transform safe, StandingsController ctrl)
    {
    ...
    private static StaffSubTab BuildSubTab(Transform parent, string label, out Button btn)
    {
        ...
        TMP_Text lbl = CreateText(go.transform, "Label", label, 22, ColorMuted, TextAlignmentOptions.Center);
    ...
    private static void BuildList(Transform safe, StandingsController ctrl)
    {
    ...
    private static GameObject BuildPersonRowTemplate(Transform parent)
    {
        ...
        SetRect(tag.GetComponent<RectTransform>(), new Vector2(0, 1), new Vector2(0, 1), new Vector2(0, 1), new Vector2(110, 34), new Vector2(20, -14));
        TMP_Text tagText = CreateText(tag.transform, "TagText", "P1", 18, Color.white, TextAlignmentOptions.Center);
        ...
        SetRect(nameText.rectTransform, new Vector2(0, 1), new Vector2(0, 1), new Vector2(0, 1), new Vector2(620, 40), new Vector2(20, -50));
        nameText.fontStyle = FontStyles.Bold;

        TMP_Text subtitle = CreateText(go.transform, "Subtitle", "Subtitle", 22, ColorMuted, TextAlignmentOptions.Left);
        SetRect(subtitle.rectTransform, new Vector2(0, 1), new Vector2(0, 1), new Vector2(0, 1), new Vector2(640, 28), new Vector2(20, -86));

        TMP_Text rating = CreateText(go.transform, "Rating", "0", 44, ColorPrimary, TextAlignmentOptions.Right);
        SetRect(rating.rectTransform, new Vector2(1, 0.5f), new Vector2(1, 0.5f), new Vector2(1, 0.5f), new Vector2(150, 56), new Vector2(-24, 0));
        ...
        SetRect(actionBtn.GetComponent<RectTransform>(), new Vector2(1, 0), new Vector2(1, 0), new Vector2(1, 0), new Vector2(220, 56), new Vector2(-24, 14));
