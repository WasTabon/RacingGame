...
public class Iteration4Setup
{
    ...
    [MenuItem("RacingGame/Iteration 4 - R&D")]
    public static void Build()
    {
        EnsureFolder("Assets/RacingGame/Scenes");

        Scene rnd = EditorSceneManager.NewScene(NewSceneSetup.EmptyScene, NewSceneMode.Single);
        BuildRnDScene();
        string rndPath = "Assets/RacingGame/Scenes/RnD.unity";
        EditorSceneManager.SaveScene(rnd, rndPath);
        ...
        if (File.Exists(staffPath)) scenes.Add(staffPath);
        scenes.Add(rndPath);
        AddScenesToBuild(scenes.ToArray());

        EditorSceneManager.OpenScene(rndPath);
        AssetDatabase.SaveAssets();
        Debug.Log("RacingGame Iteration 4 (R&D) setup complete. Open RnD scene and Play, or enter via Hub > R&D.");
    }

    private static void BuildRnDScene()
    {
        CreateCamera();
        CreateEventSystem();
        CreateManagers();
        ...
        GameObject ctrlGo = new GameObject("RnDController");
        ctrlGo.transform.SetParent(canvas.transform, false);
        ctrlGo.AddComponent<RectTransform>();
        RnDController ctrl = ctrlGo.AddComponent<RnDController>();
    ...
    private static void BuildTopBar(Transform safe, RnDController ctrl)
    {
        ...
        TMP_Text title = CreateText(hud.transform, "Title", "R&D", 46, Color.white, TextAlignmentOptions.Center);
    ...
    private static void BuildSubTabs(Transform safe, RnDController ctrl)
    {
    ...
    private static void BuildList(Transform safe, RnDController ctrl)
    {
