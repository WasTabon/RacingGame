...
public class Iteration8Setup
{
    ...
    [MenuItem("RacingGame/Iteration 8 - Finances")]
    public static void Build()
    {
        EnsureFolder("Assets/RacingGame/Scenes");

        Scene fin = EditorSceneManager.NewScene(NewSceneSetup.EmptyScene, NewSceneMode.Single);
        BuildFinanceScene();
        string finPath = "Assets/RacingGame/Scenes/Finance.unity";
        EditorSceneManager.SaveScene(fin, finPath);

        List<string> scenes = new List<string>();
        string[] prior = { "MainMenu", "Hub", "Staff", "RnD", "Car", "Base", "Weekend" };
        ...
        EditorSceneManager.OpenScene(finPath);
        AssetDatabase.SaveAssets();
        Debug.Log("RacingGame Iteration 8 (Finances) setup complete. Enter via Hub money chip, or open Finance scene and Play.");
    }

    private static void BuildFinanceScene()
    {
        CreateCamera();
        CreateEventSystem();
        CreateManagers();
        ...
        GameObject ctrlGo = new GameObject("FinanceController");
        ctrlGo.transform.SetParent(canvas.transform, false);
        ctrlGo.AddComponent<RectTransform>();
        FinanceController ctrl = ctrlGo.AddComponent<FinanceController>();

        BuildTopBar(safe.transform, ctrl);
        BuildSubTabs(safe.transform, ctrl);
        BuildOverview(safe.transform, ctrl);
        BuildSponsors(safe.transform, ctrl);
    }

    private static void BuildTopBar(Transform safe, FinanceController ctrl)
    {
        ...
        TMP_Text title = CreateText(hud.transform, "Title", "FINANCES", 42, Color.white, TextAlignmentOptions.Center);
    ...
    private static void BuildSubTabs(Transform safe, FinanceController ctrl)
    {
        ...
        string[] labels = { "OVERVIEW", "SPONSORS" };
    ...
    private static void BuildOverview(Transform safe, FinanceController ctrl)
    {
        GameObject panel = new GameObject("OverviewPanel");
        panel.transform.SetParent(safe, false);
        Image card = panel.AddComponent<Image>();
        ...
        prt.offsetMin = new Vector2(30, 30);
        prt.offsetMax = new Vector2(-30, -250);

        TMP_Text txt = CreateText(panel.transform, "OverviewText", "", 30, Color.white, TextAlignmentOptions.TopLeft);
        ...
        ctrl.overviewPanel = panel;
        ctrl.overviewText = txt;
    }

    private static void BuildSponsors(Transform safe, FinanceController ctrl)
    {
        GameObject scroll = new GameObject("SponsorsScroll");
    ...
    private static GameObject BuildPersonRowTemplate(Transform parent)
    {
        ...
        SetRect(subtitle.rectTransform, new Vector2(0, 1), new Vector2(0, 1), new Vector2(0, 1), new Vector2(660, 28), new Vector2(20, -92));
        ...
        SetRect(actionBtn.GetComponent<RectTransform>(), new Vector2(1, 0), new Vector2(1, 0), new Vector2(1, 0), new Vector2(280, 60), new Vector2(-24, 16));
